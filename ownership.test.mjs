...
test("only server role can bind; concurrent claims select one owner and reads contain no evidence", async () => {
  ...
  try {
    ...
    expect(
      (await request(app, bind, { userId: 42 }, "verification")).status,
    ).toBe(403);
    expect(
      (await request(app, bind, { ...receipt, userId: "alice" })).body.bound,
    ).toBe(false);
    await request(app, "/commerce/v1/purchases/verify", receipt);
    const claims = await Promise.all(
      ["alice", "bob"].map((userId) =>
        request(app, bind, { ...receipt, userId }),
      ),
    );
    ...
    expect(
      (await request(app, bind, { ...receipt, userId: owner })).body.bound,
    ).toBe(true);
    expect(
      (await request(app, bind, { ...receipt, userId: other })).body.bound,
    ).toBe(false);
    const access = await request(
      app,
      "/commerce/v1/entitlements?userId=" + owner,
      undefined,
      "server",
      "GET",
    );
    ...
    expect(
      (
        await request(
          app,
          "/commerce/v1/subscriptions/status?userId=" + owner,
          undefined,
          "server",
          "GET",
        )
    ...
    const injected = await request(app, "/demo/bind", {
      user: other,
      userId: owner,
    });
