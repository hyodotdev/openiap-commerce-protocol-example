...
export function openBackend(path) {
  ...
  if (
    !db
      ...
      .all()
      .some((col) => col.name === "store")
  )
    db.exec(
      "ALTER TABLE purchases ADD COLUMN store TEXT NOT NULL DEFAULT 'fixture'",
    );
  db.query("INSERT OR IGNORE INTO metadata VALUES (?,?)").run(
    "erasure-key",
    randomUUID(),
  );
  const erasureKey = db
    .query("SELECT value FROM metadata WHERE key='erasure-key'")
    .get().value;
  const marker = (userId) =>
    createHmac("sha256", erasureKey).update(userId).digest("hex");
  const erased = (userId) =>
    Boolean(
      db
  ...
  function now() {
    return db.query("SELECT value FROM settings WHERE key='clock'").get().value;
  }
  function snapshot(row) {
    return {
      ...
      state: row.state,
      active:
        ["Active", "InGracePeriod"].includes(row.state) &&
        now() < row.expires_at,
      store: row.store,
      expiresAt: row.expires_at,
      willRenew: Boolean(row.will_renew),
      ...(!row.will_renew ? { cancellationReason: "UserCanceled" } : {}),
    };
  }
  function records(userId) {
    if (erased(userId)) return [];
    const rows = db
      .query("SELECT * FROM purchases WHERE user_id=? LIMIT 101")
      .all(userId);
  ...
  function emit(eventType, row, occurredAt) {
    ...
    db.query(
      "INSERT INTO outbox (id,user_id,body,delivery_id) VALUES (?,?,?,?)",
    ).run(event.eventId, row.user_id, JSON.stringify(event), randomUUID());
    if (row.user_id)
      db.query("INSERT OR REPLACE INTO notified_gates VALUES (?,?)").run(
        row.evidence,
        Number(event.subscription.active),
      );
    return event;
  }
  ...
  const api = {
    ...
    erase(userId) {
      return db
        .transaction(() => {
          ...
          for (const row of db
            .query("SELECT evidence FROM purchases WHERE user_id=?")
            .all(userId)) {
            db.query("INSERT OR IGNORE INTO retired_purchases VALUES (?)").run(
              row.evidence,
            );
            db.query("DELETE FROM observations WHERE evidence=?").run(
              row.evidence,
            );
            db.query("DELETE FROM notified_gates WHERE evidence=?").run(
              row.evidence,
            );
          }
          db.query("DELETE FROM outbox WHERE user_id=?").run(userId);
          db.query("UPDATE purchases SET user_id=NULL WHERE user_id=?").run(
            userId,
          );
      ...
      const fixture = Object.fromEntries(
        axes.map((key) => [
          key,
          {
            ...
            notes: supported.has(key)
              ? "Fictional fixture only; no real store connected."
              : "Not implemented by this local fixture.",
          },
        ]),
      );
      return {
        ...
        bindings: { rest: "1.0" },
        implementation: {
          name: "Fresh local fixture (not a live Google provider)",
          version: "0.0.0",
        },
        eventTypes: [
          "entitlement.granted",
          ...
          google: Object.fromEntries(
            axes.map((key) => [
              key,
              {
                ...
                implementation: supported.has(key),
                notes:
                  "Google wire-format fixture and simulated lifecycle controls only. No Google API calls, RTDN, credentials, or real purchase validation.",
              },
            ]),
          ),
        },
    ...
    bind(input) {
      ...
      return db
        .transaction(() => {
          const row = db
            .query("SELECT * FROM purchases WHERE evidence=?")
            .get(receipt);
          if (
            erased(input.userId) ||
            db
              .query("SELECT evidence FROM retired_purchases WHERE evidence=?")
              .get(receipt)
          )
            return { bound: false };
          if (!row || (row.user_id && row.user_id !== input.userId))
            return { bound: false };
          db.query("UPDATE purchases SET user_id=? WHERE evidence=?").run(
            input.userId,
            receipt,
          );
    ...
    expire(userId) {
      return db
        .transaction(() => {
          ...
          if (
            db.query("SELECT id FROM observations WHERE id=?").get(observation)
          )
            return { changed: false };
          const previousGate = db
            .query("SELECT active FROM notified_gates WHERE evidence=?")
            .get(row.evidence)?.active;
          db.query(
            "UPDATE purchases SET state='Expired',will_renew=0 WHERE evidence=?",
          ).run(row.evidence);
          db.query("INSERT INTO observations VALUES (?,?)").run(
            observation,
            row.evidence,
          );
          const expired = { ...row, state: "Expired", will_renew: 0 };
          emit("subscription.expired", expired, row.expires_at);
          if (previousGate && row.user_id)
            emit("entitlement.revoked", expired, row.expires_at);
          return { changed: true };
        })
        .immediate();
    },
      ...
      return db
        .transaction(() => {
          ...
          if (
            ...
            return { changed: false };
          db.query("UPDATE purchases SET will_renew=0 WHERE evidence=?").run(
            row.evidence,
          );
          db.query("INSERT INTO observations VALUES (?,?)").run(
            observation,
          ...
          if (previousGate && !event.subscription.active && row.user_id)
            emit("entitlement.revoked", { ...row, will_renew: 0 }, now());
    ...
    verify(input) {
      ...
      if (isValid)
        ...
        ).run(key, END, input.store);
      return {
        store: input.store,
        isValid,
        state: isValid ? "ENTITLED" : "INAUTHENTIC",
        ...(isValid ? { productId: PRODUCT } : {}),
        environment: "sandbox",
      };
    },
    state(userId = "alice") {
      return {
        ...
        now: db.query("SELECT value FROM settings WHERE key='clock'").get()
          .value,
        purchases: db.query("SELECT count(*) AS count FROM purchases").get()
          .count,
        queued: db
          .query("SELECT count(*) AS count FROM outbox WHERE status='pending'")
          .get().count,
        access: api.status({ userId }).active,
        message: erased(userId)
          ? "This test account has been erased. Its old session and purchase can no longer create access."
          : api.status({ userId }).active
            ? api.status({ userId }).subscription.willRenew
              ? "Premium is open. Cancel renewal to see why paid access continues."
              : "Renewal is canceled. Premium stays open until October 13."
            : api.status({ userId }).subscription
              ? "The paid period has ended. Premium is locked, including after a server restart."
              : "This customer has no current paid access. Verify the receipt, then connect it to Alice.",
      };
    },
    close() {
      db.close();
