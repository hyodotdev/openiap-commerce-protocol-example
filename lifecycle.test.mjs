...
test("cancellation keeps paid access and atomically saves one observation and immutable event", async () => {
  ...
  try {
    ...
    db.exec(
      "CREATE TRIGGER reject_event BEFORE INSERT ON outbox BEGIN SELECT RAISE(ABORT, 'test disk failure'); END;",
    );
    const fail = await request(app, "/fixture/cancel", { userId: "alice" });
    expect(fail.status).toBe(500);
    expect(app.backend.status({ userId: "alice" }).subscription.willRenew).toBe(
      true,
    );
    ...
    expect(
      (await request(app, "/fixture/cancel", { userId: "alice" })).body.changed,
    ).toBe(true);
    const bytes = db.query("SELECT body FROM outbox ORDER BY rowid").all();
    expect(bytes).toHaveLength(2);
    expect(
      bytes.every((row) =>
        valid("#/$defs/CommerceEvent", JSON.parse(row.body)),
      ),
    ).toBe(true);
    ...
    expect(db.query("SELECT body FROM outbox ORDER BY rowid").all()).toEqual(
      bytes,
    );
  } finally {
    await app.close();
