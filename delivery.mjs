...
export function verifySignature({ body, timestamp, signature, secrets, now }) {
  if (
    !/^(0|[1-9][0-9]*)$/.test(String(timestamp)) ||
  ...
  const candidates = signature
    ...
    .filter((value) => /^v1=[a-f0-9]{64}$/.test(value));
...
export function startReceiver({
  ...
  db.exec(
    "PRAGMA journal_mode=WAL; CREATE TABLE IF NOT EXISTS inbox (project_id TEXT NOT NULL, event_id TEXT NOT NULL, user_id TEXT, body TEXT NOT NULL, PRIMARY KEY(project_id,event_id));",
  );
  db.exec(
    "CREATE TABLE IF NOT EXISTS metadata (key TEXT PRIMARY KEY,value TEXT NOT NULL); CREATE TABLE IF NOT EXISTS erased_users (marker TEXT PRIMARY KEY);",
  );
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      if (
        request.method !== "POST" ||
        new URL(request.url).pathname !== "/webhooks/commerce"
      )
      ...
      if (typeof event?.eventVersion !== "string")
        return new Response(null, { status: 400 });
      if (event.eventVersion.split(".")[0] !== "1")
      ...
      if (state.loseAckNext) {
        state.loseAckNext = false;
        return new Response(null, { status: 503 });
      }
...
export function createDelivery(
  ...
  if (
    ...
    url.pathname !== "/webhooks/commerce"
  )
    throw new Error(
      "This local fixture only delivers to its loopback receiver",
    );
  let running;
  async function sendDue() {
    const rows = backend.db
      .query(
        "SELECT * FROM outbox WHERE status='pending' AND next_attempt<=? ORDER BY rowid",
      )
      .all(clock());
    const trace = [];
    for (const row of rows) {
      if (!backend.db.query("SELECT id FROM outbox WHERE id=?").get(row.id))
        continue;
      const timestamp = Math.floor(clock() / 1000);
      const headers = {
        "Content-Type": "application/json",
        "openiap-timestamp": String(timestamp),
        "openiap-signature": sign(WEBHOOK_SECRET, timestamp, row.body),
        "openiap-event-id": row.id,
        "openiap-delivery-id": row.delivery_id,
      };
      let status;
      try {
      ...
      backend.db
        .query(
          "UPDATE outbox SET attempts=?,status=?,next_attempt=? WHERE id=?",
        )
        .run(attempts, outcome, clock() + 1000 * 2 ** (attempts - 1), row.id);
      trace.push({
        method: "POST",
        destination,
        headers,
  ...
  return {
    ...
    async drain() {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const next = backend.db
          .query(
            "SELECT min(next_attempt) at FROM outbox WHERE status='pending'",
          )
          .get().at;
        if (next === null) break;
