...
test("published signature and response vectors pass against the new transport", () => {
  for (const v of signatureVectors.cases) {
    ...
    expect(
      secrets.map((secret) => sign(secret, v.timestamp, v.body)).join(","),
    ).toBe(v.expected);
    for (const secret of secrets)
      expect(
        verifySignature({
          ...
          signature: v.presentedHeader ?? v.expected,
          secrets: [secret],
          now: v.timestamp,
        }),
      ).toBe(true);
  }
  for (const v of signatureVectors.rejections)
    expect(
      verifySignature({
        body: v.body,
        timestamp: v.timestamp,
        signature: v.presentedSignature,
        secrets: [v.secret],
        now: v.receiverNow ?? v.timestamp,
      }),
    ).toBe(false);
  for (const v of signatureVectors.responseSemantics.cases)
    expect(classify(v.status)).toBe(v.action);
...
test("real HTTP retry and lost acknowledgement retain bytes and IDs, deduplicate, reject tampering, and dead-letter", async () => {
  ...
  try {
    ...
    expect(second[0].headers["openiap-delivery-id"]).toBe(
      first[0].headers["openiap-delivery-id"],
    );
    expect(second[0].body).toEqual(first[0].body);
    expect(second[0].headers["openiap-signature"]).not.toBe(
      first[0].headers["openiap-signature"],
    );
    clock += 2001;
    ...
    const tampered = await fetch(app.receiver.url, {
      method: "POST",
      headers: third[0].headers,
      body: JSON.stringify({ ...third[0].body, userId: "bob" }),
    });
    expect(tampered.status).toBe(401);
    const row = app.backend.db.query("SELECT * FROM outbox LIMIT 1").get();
    app.backend.db
      .query(
        "UPDATE outbox SET status='pending', attempts=0, next_attempt=0 WHERE id=?",
      )
      .run(row.id);
    for (let i = 0; i < 4; i++) {
      app.receiver.state.failNext = true;
    ...
    expect(
      app.backend.db.query("SELECT status FROM outbox WHERE id=?").get(row.id)
        .status,
    ).toBe("dead-letter");
    expect(app.receiver.db.query("SELECT count(*) n FROM inbox").get().n).toBe(
      2,
    ...
    const res = await fetch(app.receiver.url, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "openiap-timestamp": String(Math.floor(clock / 1000)),
        "openiap-signature": "v1=bad",
      },
      body: bytes,
    });
