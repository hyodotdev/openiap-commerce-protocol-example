...
export function openBackend(path) {
  ...
  function snapshot(row) {
    return { productId: PRODUCT, state: row.state, active: ['Active', 'InGracePeriod'].includes(row.state) && now() < row.expires_at, store: 'fixture', expiresAt: row.expires_at, willRenew: Boolean(row.will_renew) };
  }
  ...
  const api = {
    ...
    state(userId = 'alice') {
      return {
        ...
        message: api.status({ userId }).active ? 'This customer owns the verified purchase and can use Premium.' : 'This customer has no current paid access. Verify the receipt, then connect it to Alice.',
      };
    },
    close() { db.close(); },
  };
